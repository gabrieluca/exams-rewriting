...
img = cv2.imread("database_files/exams/NM7510_P3_2019_2/NM7510_P3_pacote_A_2_sem_2019-34.png",1) 
...
if pt[0]>1600:
    img1 = img[int(pt[0]/50)+int(w*1.8):rt[1] + int(f/1.6), pt[0]+int(w*0.7):rt[0]+int(w*0.5)]
    img2 = img[int(pt[0]/50)+w+1630:rt[1] + int(f/2.2), pt[0]-1625:rt[0]+w-1685]
    ...
    if rt[0]<1650:
        img1 = img[int(pt[0]/50)+w+1525:rt[1] + int(f*0.5), pt[0]+int(w*0.4):rt[0]+int(w/2.5)]
        img2 = img[int(pt[0]/50)+int(w*1.8):rt[1] + int(f*0.5), pt[0]+1680:rt[0]+w+1625]
        img1 = cv2.resize(img1, (1080, 660))
        img2 = cv2.resize(img2, (1080, 660))
        cv2.imshow("img", img1)
        cv2.imshow("img2", img2)
        ...
        img1 = img[int(pt[0]/50)+w+1660:rt[1] + int(f*0.45), pt[0]+int(w*0.5):rt[0]+int(w/1.5)-1658]
