...
def main_impar():
    ...
    try:
        os.mkdir("C:/Users/Windows/Documents/GitHub/exams-system/Images/Answers/Contour/{0}_{1}".format(t, n))
        print("Directory ",t,"_",n," Created ") 
    except FileExistsError:
        print("Directory " , t ,"_", n ,  " already exists")
    path = "C:/Users/Windows/Documents/GitHub/exams-system/Images/Answers/Contour/{0}_{1}".format(t, n)

    # Save images in the target Directory
    cv2.imwrite(os.path.join(path, 'Area_resolucao_folha_1({0}).png'.format(n)), ares1)
    cv2.imwrite(os.path.join(path, 'Area_resolucao_folha_2({0}).png'.format(n)), ares2)
    ...
    for c in contours:
        ...
        if a < 30 or b < 150:
            ...
            cv2.imwrite(os.path.join(path, '{0}({1}).png'.format(questoes[0+f],n)), resp)
            cv2.waitKey(0)
            f+=1
    cv2.destroyAllWindows()

if p == 0:
    main_par(img,template,template2,t,n)
else:
    main_impar()